...
class ESearch {
	...
	public function __construct( $connection ){
		// Check connection
		if( $connection ) // Set connection class var
			$this->connection = $connection;
		else // Print error if not connect
			throw new EException("Неверное подключение к базе данных. Инициализируйте его правильно.");
	...
	public function setColumns( $columns = Array( ) ){
		...
		if( !count( $columns ) )
			throw new EException("Укажите FULLTEXT TYPE поля.");
	...
	/*
	 * 
		$SearchObjectString = 'MATCH (`Description`,`NamePrefix`,`NameName`,`AdressOther`,`Keywords`) AGAINST (:search IN BOOLEAN MODE)';
		$PublicList = $this->db->createCommand(
				"SELECT *,{$SearchObjectString} as relevance, CASE when NameName like '%".$LikeStemmer."%' then 1 else 0 END as titlematch FROM {{public}} WHERE {$SearchObjectString} ORDER BY titlematch DESC, `relevance` DESC limit ".$start.','.$this->listcount)
					->safeParam(':search',Tools::SearchString( $this->url->p['squery'] ))
					->safeParam(':slike',$LikeStemmer )->getArray();
	 */
	...
	public function exec( ){
		...
		if( count( $this->columns ) > 0 ){
			...
			throw new EException("Прежде чем начать поиск следует указать поля в полнотектовом индексе.");
		
		if( !$this->table )
			throw new EException("Прежде чем начать поиск следует указать таблицу.");
		...
		if ( mysql_errno( $this->connection ) )
			throw new EException("Неудалось выполнить запрос. Ошибка MySQL: ". mysql_error( $this->connection ));
	...
	public function setImportantFields( $arrayFields ){
		...
		if( gettype( $arrayFields ) != 'array' ) 
			throw new EException("Следует передавать важные поля как массив.");
	...
	public function dataType( $type = 'assoc' ){
		...
		if( in_array( $type, $this->typeIsset ) ) $this->typeSet = $type;
			...
			throw new EException("Такого типа данных `{$type}` нет.");
